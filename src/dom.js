...
const generateList = () => {
  ...
  if (activeProject.items) {
    for (let i = 0; i < activeProject.items.length; i++) {
      ...
      itemCheckboxDiv.onclick = function () {
        toggleComplete(activeProject, i);
      };
      ...
      item.onclick = function () {
        editListItem(activeProject, i);
      };
      itemContainer.appendChild(item);

      // Append due date to item container
      const itemDueDate = document.createElement("p");
      const itemDueDateText = document.createTextNode(
        `${activeProject.items[i].dueDate}`
      );
      ...
      itemDeleteButtonDiv.onclick = function () {
        deleteListItem(activeProject, i);
      };
...
const generateProjectList = () => {
  // Elements
  const sidebarProjectHolder = document.querySelector(
...
const addNewProject = () => {
  ...
  if (document.querySelector(".add-project-new-wrapper")) {
    // Elements
    const addProjectNewWrapper = document.querySelector(
  ...
  projectFormSubmitButton.setAttribute(
    "class",
...
const toggleMenu = () => {
  ...
  const trigger = function () {
    ...
    wrapper.classList.toggle("menu-closed");
  };

  headerMenuButton.addEventListener("click", trigger);
};

const closeMobileMenuDefault = () => {
  // Elements
  const sidebar = document.querySelector(".sidebar-container");
